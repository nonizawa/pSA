...
def annealing(tau, I0_min, I0_max, beta, nrnd, Mshot, J_matrix, spin_vector, Itanh_ini, rand_type):
    Itanh = Itanh_ini
...
for k in range(trial):
    ...
    (last_spin_vector, annealing_time) = annealing(tau, I0_min, I0_max, beta, nrnd, Mshot, J_matrix, ini_spin_vector, Itanh_ini, rand_type)
...
# Output file
data = [
    name[0],name[1],name[2],name[3],name[4], gamma, delta, rand_type, cut_average, cut_max, cut_min, 100*cut_average/best_known, 100*cut_max/best_known, time_average]

if os.path.isfile("./result/result_pSA.csv"): # "result.csv" ファイルが存在する場合
    with open("./result/result_pSA.csv", 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(data)
else: # "result_cpu.csv" ファイルが存在しない場合
    with open("./result/result_pSA.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['graph','#node', 'Weights','Structure','Best-known value',	'gamma','delta','rand_type','mean','max','min','mean%','max%','sim_time'])
        writer.writerow(data)
